...
### Fixed
- Restored the VS Code webview bundle in the packaged VSIX so the MD Feedback sidebar no longer renders as a blank or black panel.
- Fixed the Windows webview build path used during VS Code extension packaging.

### Build / Release Safety
- VSIX packaging now fails if required webview assets are missing from the extension package.
- VS Code extension packaging now always runs a full build before generating the VSIX.
