...
### Fixed
- Restored the packaged webview bundle so the MD Feedback sidebar no longer renders as a blank or black panel in VS Code.
- Fixed the Windows webview build path used during extension packaging.

### Build / Release Safety
- Packaging now fails if required webview assets are missing from the VSIX.
- Packaging now always runs a full build before generating the VSIX.
