...
**설정**에서 타이밍 동작을 조정할 수 있습니다 (`md-feedback.*`):

- `md-feedback.autoCheckpointIntervalMs` (기본값: `600000`)
- `md-feedback.sectionTrackDebounceMs` (기본값: `3000`)
- `md-feedback.editorSwitchDebounceMs` (기본값: `150`)
- `md-feedback.fileWatchDebounceMs` (기본값: `500`)

워크스페이스가 매우 크거나 백그라운드 업데이트 빈도를 낮추고 싶다면 값을 더 크게 설정하세요.
...
- 신규 통합은 `MemoV2` 기반 API(`splitDocument`, `extractMemosV2`)를 사용하세요.
- 레거시 헬퍼는 호환성 전용이며 `legacy.*` 경로로만 제공합니다.
- deprecated `Memo` 타입 흐름에 대한 신규 의존은 추가하지 마세요.
